...
app.on('ready', function() {
  ...
  parent.respondTo('browser-initialize', function(opts, done) {
    ...
    function winOn (name, details) {
      if(!closed) {
        parent.emit.apply(parent, [name, details || {}]);
      }
    }

    win.on('page-title-updated', function(e, title) {
       winOn('page-title-updated', {title: title})
    })
    win.on('close', function(e) { winOn('close') })
    win.on('closed', function(e) { winOn('closed') })
    win.on('session-end', function(e) { winOn('session-end') })
    win.on('unresponsive', function(e) { winOn('unresponsive') })
    win.on('responsive', function(e) { winOn('responsive') })
    win.on('blur', function(e) { winOn('blur') })
    win.on('focus', function(e) { winOn('focus') })
    win.on('show', function(e) { winOn('show') })
    win.on('hide', function(e) { winOn('hide') })
    win.on('ready-to-show', function(e) { winOn('ready-to-show') })
    win.on('maximize', function(e) { winOn('maximize') })
    win.on('unmaximize', function(e) { winOn('unmaximize') })
    win.on('minimize', function(e) { winOn('minimize') })
    win.on('restore', function(e) { winOn('restore') })
    win.on('resize', function(e) { winOn('resize') })
    win.on('enter-full-screen', function(e) { winOn('enter-full-screen') })
    win.on('enter-html-full-screen', function(e) { winOn('enter-html-full-screen') })
    win.on('leave-full-screen', function(e) { winOn('leave-full-screen') })
    win.on('leave-html-full-screen', function(e) { winOn('leave-html-full-screen') })
    ...
    win.webContents.on('will-prevent-unload', forward('will-prevent-unload'));
    win.webContents.on('before-input-event', forward('before-input-event'));
    win.webContents.on('devtools-opened', forward('devtools-opened'));
    win.webContents.on('devtools-closed', forward('devtools-closed'));
    win.webContents.on('devtools-focused', forward('devtools-focused'));
    win.webContents.on('login', forward('login'));
    win.webContents.on('found-in-page', forward('found-in-page'));
    win.webContents.on('did-change-theme-color', forward('did-change-theme-color'));
    win.webContents.on('update-target-url', forward('update-target-url'));
    win.webContents.on('cursor-changed', forward('cursor-changed'));
    win.webContents.on('context-menu', forward('context-menu'));
    win.webContents.on('select-bluetooth-device', forward('select-bluetooth-device'));
    win.webContents.on('devtools-reload-page', forward('devtools-reload-page'));
    win.webContents.on('will-attach-webview', forward('will-attach-webview'));
    win.webContents.on('did-attach-webview', forward('did-attach-webview'));
   ...
   * getWindowState
   */
  parent.respondTo('getWindowState', function(done) {
    var state = {
      isFocused: win.isFocused && win.isFocused() || null,
      isDestroyed: win.isDestroyed && win.isDestroyed() || null,
      isVisible: win.isVisible && win.isVisible() || null,
      isModal: win.isModal && win.isModal() || null,
      isMaximized: win.isMaximized && win.isMaximized() || null,
      isMinimized: win.isMinimized && win.isMinimized() || null,
      isFullscreen: win.isFullScreen && win.isFullScreen() || null,
      bounds: win.getBounds && win.getBounds() || null,
      contentBounds: win.getContentBounds && win.getContentBounds() || null,
      size: win.getSize && win.getSize() || null,
      contentSize: win.getContentSize && win.getContentSize() || null,
      minimumSize: win.getMinimumSize && win.getMinimumSize() || null,
      maximumSize: win.getMaximumSize && win.getMaximumSize() || null,
      isResizable: win.isResizable && win.isResizable() || null,
      isMovable: win.isMovable && win.isMovable() || null,
      isMinimizable: win.isMinimizable && win.isMinimizable() || null,
      isMaximizable: win.isMaximizable && win.isMaximizable() || null,
      isFullScreenable: win.isFullScreenable && win.isFullScreenable() || null,
      isClosable: win.isClosable && win.isClosable() || null,
      isAlwaysOnTop: win.isAlwaysOnTop && win.isAlwaysOnTop() || null,
      getPosition: win.getPosition && win.getPosition() || null,
      getTitle: win.getTitle && win.getTitle() || null,
      isKiosk: win.isKiosk && win.isKiosk() || null,
      getOpacity: win.getOpacity && win.getOpacity() || null
    };
    done(null , state);
  });
   ...
   * setWindowOptions
   */
  parent.respondTo('setWindowOptions', function(options, done) {
    if (options) {
      if (win.focus && options.focus) {
        win.focus();
      }
      if (win.blur && options.blur) {
        win.blur();
      }
      if (win.show && options.show) {
        win.show();
      }
      if (win.hide && options.hide) {
        win.hide();
      }
      if (win.maximize && options.maximize) {
        win.maximize();
      }
      if (win.minimize && options.minimize) {
        win.minimize();
      }
      if (win.setFullScreen && options.hasOwnProperty('fullscreen')) {
        win.setFullScreen(options.fullscreen);
      }
      if (win.setBounds && options.hasOwnProperty('bounds')) {
        win.setBounds(options.bounds);
      }
      if (win.setContentBounds && options.hasOwnProperty('contentBounds')) {
        win.setContentBounds(options.contentBounds);
      }
      if (win.setSize && options.hasOwnProperty('size') &&
          options.size.hasOwnProperty('width') &&
          options.size.hasOwnProperty('height')) {
        win.setSize(options.size.width, options.size.height);
      }
      if (win.setContentSize && options.hasOwnProperty('contentSize') &&
          options.contentSize.hasOwnProperty('width') &&
          options.contentSize.hasOwnProperty('height')) {
        win.setContentSize(options.contentSize.width, options.contentSize.height);
      }
      if (win.setMinimumSize && options.hasOwnProperty('minimumSize') &&
          options.minimumSize.hasOwnProperty('width') &&
          options.minimumSize.hasOwnProperty('height')) {
        win.setMinimumSize(options.minimumSize.width, options.minimumSize.height);
      }
      if (win.setMaximumSize && options.hasOwnProperty('maximumSize') &&
          options.maximumSize.hasOwnProperty('width') &&
          options.maximumSize.hasOwnProperty('height')) {
        win.setMaximumSize(options.maximumSize.width, options.maximumSize.height);
      }
      if (win.setResizable && options.hasOwnProperty('resizable')) {
        win.setResizable(options.resizable);
      }
      if (win.setMovable && options.hasOwnProperty('movable')) {
        win.setMovable(options.movable);
      }
      if (win.setMinimizable && options.hasOwnProperty('minimizable')) {
        win.setMinimizable(options.minimizable);
      }
      if (win.setMaximizable && options.hasOwnProperty('maximizable')) {
        win.setMaximizable(options.maximizable);
      }
      if (win.setFullScreenable && options.hasOwnProperty('fullscreenable')) {
        win.setFullScreenable(options.fullscreenable);
      }
      if (win.setClosable && options.hasOwnProperty('closable')) {
        win.setClosable(options.closable);
      }
      if (win.setAlwaysOnTop && options.hasOwnProperty('alwaysontop')) {
        win.setAlwaysOnTop(options.alwaysontop);
      }
      if (win.center && options.center) {
        win.center(options.center);
      }
      if (win.setPosition && options.hasOwnProperty('position') &&
          options.position.hasOwnProperty('x') &&
          options.position.hasOwnProperty('y')) {
        win.setPosition(options.position.x, options.position.y);
      }
      if (win.setTitle && options.hasOwnProperty('title')) {
        win.setTitle(options.title);
      }
      if (win.flashFrame && options.hasOwnProperty('flashframe')) {
        win.flashFrame(options.flashframe);
      }
      if (win.setKiosk && options.hasOwnProperty('kiosk')) {
        win.setKiosk(options.kiosk);
      }
      if (win.setOpacity && options.hasOwnProperty('opacity')) {
        win.setOpacity(options.opacity);
      }
      if (win.setFocusable && options.hasOwnProperty('focusable')) {
        win.setFocusable(options.focusable);
      }
    }
    done();
  });
