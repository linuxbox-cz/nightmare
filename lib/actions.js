...
/**
 * Determine if a xpath is visible on a page.
 *
 * @param {String} xpath
 * @param {Function} done
 */

exports.visibleXpath = function(xpath, done) {
  debug('.visibleXpath() for ' + xpath)
  this.actionLogFnc('visibleXpath', {
    xpath: xpath
  })
...
/**
 * Determine if a xpath exists on a page.
 *
 * @param {String} xpath
 * @param {Function} done
 */

exports.existsXpath = function(xpath, done) {
  debug('.existsXpath() for ' + xpath)
  this.evaluate_now(
    function(xpath) {
...
exports.clickXpath = function(xpath, done) {
  debug('.clickXpath() on ' + xpath)
  // pavrin start
  this.actionLogFnc('clickXpath', {
    xpath: xpath
  })
  // pavrin end
  this.evaluate_now(
    function (xpath) {
      ...
      if (!element) {
        throw new Error('Unable to find element by xpath: ' + xpath)
      }
      var bounding = element.getBoundingClientRect()
      var event = new MouseEvent('click', {
        view: document.window,
...
exports.pointerdown = function(selector, done) {
  debug('.pointerdown() on ' + selector)
  this.evaluate_now(
    function(selector) {
      document.activeElement.blur()
      var element = document.querySelector(selector)
...
/**
 * PointerDownXpath on element.
 *
 * @param {String} selector
 * @param {Function} done
 */

exports.pointerdownXpath = function(selector, done) {
  debug('.pointerdownXpath() on ' + selector)
  this.evaluate_now(
    function(selector) {
      document.activeElement.blur()
...
exports.mousedownXpath = function(xpath, done) {
  debug('.mousedownXpath() on ' + xpath)
  // pavrin start
  this.actionLogFnc('mousedownXpath', {
    xpath: xpath
  })
  // pavrin end
  this.evaluate_now(
...
exports.mouseupXpath = function(xpath, done) {
  debug('.mouseupXpath() on ' + xpath)
  this.evaluate_now(
    function (xpath) {
      var element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue
      if (!element) {
...
exports.mouseoverXpath = function(xpath, done) {
  debug('.mouseoverXpath() on ' + xpath)
  this.evaluate_now(
    function (xpath) {
      var element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue
      if (!element) {
...
exports.mouseoutXpath = function(xpath, done) {
  debug('.mouseoutXpath() on ' + xpath)
  // pavrin start
  this.actionLogFnc('mouseoutXpath', {
    xpath: xpath
  })
  // pavrin end
  this.evaluate_now(
...
var focusXpath = function(done, xpath) {
  return this.evaluate_now(
    function(xpath) {
      var element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue
      if (element) {
...
var blurXpath = function(done, xpath) {
  return this.evaluate_now(
    function(xpath) {
      //it is possible the element has been removed from the DOM
      //between the action and the call to blur the element
...
exports.typeXpath = function() {
  var xpath = arguments[0], text, done
  if(arguments.length == 2) {
    done = arguments[1]
  } else {
    text = arguments[1]
  ...
  debug('.typeXpath() %s into %s', text, xpath)
  var self = this

  focusXpath.bind(this)(function(err) {
    if(err) {
      debug('Unable to .typeXpath() into non-existent xpath %s', xpath)
      return done(err)
    }

    var blurDone = blurXpath.bind(this, done, xpath)
    if ((text || '') == '') {
      this.evaluate_now(
        function(xpath) {
...
exports.insertXpath = function(xpath, text, done) {
  if (arguments.length === 2) {
    done = text
    text = null
  }

  debug('.insertXpath() %s into %s', text, xpath)
  // pavrin start
  this.actionLogFnc('insertXpath', {
    xpath: xpath,
    text: text
  })
  // pavrin end
  ...
  focusXpath.bind(this)(function(err) {
    if(err) {
      debug('Unable to .insertXpath() into non-existent xpath %s', xpath)
      return done(err)
    }
...
exports.checkXpath = function(xpath, done) {
  debug('.checkXpath() ' + xpath)
  this.evaluate_now(
    function(xpath) {
      var element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue
      var event = document.createEvent('HTMLEvents')
...
exports.uncheckXpath = function(xpath, done){
  debug('.uncheckXpath() ' + xpath)
  this.evaluate_now(
    function(xpath) {
      var element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue
      var event = document.createEvent('HTMLEvents')
...
exports.selectXpath = function(xpath, option, done) {
  debug('.selectXpath() ' + xpath)
  this.evaluate_now(
    function(xpath, option) {
      var element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue
      var event = document.createEvent('HTMLEvents')
      element.value = option
      event.initEvent('change', true, true)
...
exports.waitXpath = function () {
  var args = sliced(arguments);
  var done = args[args.length-1];
  if (args.length < 2) {
    debug('Not enough arguments for .waitXpath()');
    return done();
  }
  ...
  if (typeof arg === 'string') {
    ...
    debug('.waitXpath() for '+arg+' element'+(timeout ? ' or '+timeout+'msec' : ''));
    waitelemXpath.apply({ timeout: timeout }, [this, arg, done]);
  }
  else {
    done();
  }
...
function waitelemXpath(self, xpath, done) {
  debug('.waitelemXpath() ' + xpath)
  var elementPresent
  eval(
    'elementPresent = function() {' +
  ...
  var newDone = function(err) {
    if (err) {
      return done(
        new Error(
          `.wait() for ${xpath} timed out after ${
            self.options.waitTimeout
          }msec`
        )
      )
...
exports.getWindowState = function (done) {
  debug('.getWindowState()');
  this.child.call('getWindowState', function (error, state) {
    done(null, state);
  });
};
...
exports.setWindowOptions = function (options, done) {
  debug('.setWindowOptions()');
  this.child.call('setWindowOptions', options, done);
};
